...
for triple in "${!TRIPLES_ARRAY[@]}"; do
    ...
    COMPILER_FLAGS="--target=$TRIPLE $MARCH -fno-omit-frame-pointer -nostdlib"

    cmake \
        -G Ninja \
        -B build-compiler_rt-$TRIPLE                    \
    ...
    cmake --build build-compiler_rt-$TRIPLE;
    cmake --install build-compiler_rt-$TRIPLE;
done
popd
